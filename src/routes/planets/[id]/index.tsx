...
import PlanetCardInfo from "~/components/planet-card-info/planet-card-info"
  ...
  return planetInfo ? (
    <section class="text-white grid grid-cols-12 gap-4 gap-y-0 mt-[69px] md:gap-y-0 md:mt-0 md:mx-6 lg:gap-y-0 lg:gap-8 lg:mx-8">
      ...
      <PlanetInfo
        ...
        name={planetInfo.value.name}
      />

      <PlanetCardInfo planet={planetInfo.value} />
    </section>
  ) : (
    <LoadingPlanet />
  )
...
export const head: DocumentHead = ({ resolveValue }) => {
  const planet = resolveValue(usePlanet)
  ...
  return {
    ...
    meta: [
      {
        name: "description",
        content: `Info about the ${planet.name} planet`,
      },
      {
        name: "id",
        content: planet.id,
